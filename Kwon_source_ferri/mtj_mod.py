...
def mtj_mod(init,phi_init,t_step,v_pulse,t_pulse,t_relax,Happl,Hshe,J_stt,J_she,vhold,xi):
    ...
    e = 1.6e-19                
    kb = 1.38e-23
    M_a = 6.25e5 * u0  # Magnetization of layer A
    M_b = 1.2e4 * u0  # Magnetization of layer B

    gamma_a = 2.2e5  # Gyromagnetic ratio for layer A
    gamma_b = 6.0e5  # Gyromagnetic ratio for layer B
    # gamma = (M_a - M_b) / (M_a / gamma_a + M_b / gamma_b)        
    # gamma = 2*u0*uB/h_bar
    gamma = 5.01e5
    print("gamma = ", gamma)
    gamma_b = gamma/u0
    ...
    alpha_a = 1.0  # Damping factor for layer A
    alpha_b = 0.03  # Damping factor for layer B     
    # alpha = ((alpha_a * M_a) / gamma_a + (alpha_b * M_b) / gamma_b) / (M_a / gamma_a + M_b / gamma_b)
    alpha = 1
    ...
    T = 300
    Ki = 1.0056364e-3
    Ki = Ki - u0 * (M_a - M_b) ** 2 / 2
    print("Ki = ", Ki)
    Ms = M_a - M_b  # Effective magnetization
    Bsat = Ms*u0
    ksi = 75e-15
