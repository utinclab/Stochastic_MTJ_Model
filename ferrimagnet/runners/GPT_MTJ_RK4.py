...
if m_trajectory.shape[0] > 0: # Check if trajectory exists
    ...
    start_arrow = ax.quiver(0, 0, 0, m_trajectory[0, 0], m_trajectory[0, 1], m_trajectory[0, 2], color="red", length=np.linalg.norm(m_trajectory[0]), normalize=False, label="Start Vec")
...
# The 'frame_idx' passed here will be 0, 10, 20, ... (or whatever skip rate is chosen)
def update(frame_idx):
    # Update the trajectory line up TO the current index
    # Slicing with [:frame_idx+1] ensures the line grows
    line.set_data(m_trajectory[:frame_idx+1, 0], m_trajectory[:frame_idx+1, 1])
    line.set_3d_properties(m_trajectory[:frame_idx+1, 2])

    # Update the start arrow to show the vector AT the current index 'frame_idx'
    # Ensure start_arrow was initialized successfully
    if 'start_arrow' in locals() and m_trajectory.shape[0] > frame_idx:
         current_m = m_trajectory[frame_idx]
         # Quiver update needs segments: [[[x, y, z], [u, v, w]], ...]
         start_arrow.set_segments([[[0, 0, 0], current_m]])
    
    # Return the artists that need to be redrawn
    # End arrow is static, no need to return unless you animate it too
    return line, start_arrow
